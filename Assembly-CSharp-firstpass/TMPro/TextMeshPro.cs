...
namespace TMPro
{
	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		protected override void Awake()
		{
			...
			{
				global::Debug.LogWarning("Converting Vertex Colors from Color32 to Color.", this);
		...
		protected override int SetArraySizes(int[] chars)
		{
			...
			while (chars[num3] != 0)
			{
				...
				if (!this.m_currentFontAsset.characterDictionary.TryGetValue(num4, out tmp_Glyph))
				{
					...
					if (tmp_Glyph == null)
					{
						...
						if (this.m_currentFontAsset.characterDictionary.TryGetValue(num5, out tmp_Glyph))
						{
							...
							{
								global::Debug.LogWarning("Character with ASCII value of " + num4 + " was not found in the Font Asset Glyph Table.", this);
							}
							...
							if (tmp_Glyph == null)
							{
								...
								else
								{
									...
									else if (this.m_currentFontAsset.characterDictionary.TryGetValue(32, out tmp_Glyph))
									{
										if (!TMP_Settings.warningsDisabled)
										{
											global::Debug.LogWarning("Character with ASCII value of " + num4 + " was not found in the Font Asset Glyph Table. It was replaced by a space.", this);
										}
										num4 = (chars[num3] = 32);
									}
								}
				...
				this.m_textInfo.characterInfo[this.m_totalCharacterCount].materialReferenceIndex = this.m_currentMaterialIndex;
				this.m_materialReferences[this.m_currentMaterialIndex].isFallbackFont = flag;
			...
			for (int l = 0; l < num6; l++)
			{
				if (l > 0)
				{
					...
					{
						Material material = this.m_materialReferences[l].material;
						Material fallbackMaterial = this.m_subTextObjects[l].m_fallbackMaterial;
						if (material != fallbackMaterial)
						{
							TMP_MaterialManager.RemoveFallbackMaterialReference(fallbackMaterial);
							this.m_subTextObjects[l].m_fallbackMaterial = material;
			...
			int num7 = num6;
			while (num7 < this.m_subTextObjects.Length + 1 && this.m_subTextObjects[num7] != null)
			{
		...
		protected override void GenerateTextMesh()
		{
			...
			else
			{
				...
				case TextAlignmentOptions.TopJustified:
					...
					{
						vector7 = textContainerLocalCorners[1] + new Vector3(0f + margin.x, 0f - this.m_maxAscender - margin.y, 0f);
					}
					else
					{
						vector7 = textContainerLocalCorners[1] + new Vector3(0f + margin.x, 0f - this.m_textInfo.pageInfo[num6].ascender - margin.y, 0f);
					}
					break;
				...
				case TextAlignmentOptions.Justified:
					if (this.m_overflowMode != TextOverflowModes.Page)
					{
						vector7 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (this.m_maxAscender + margin.y + num10 - margin.w) / 2f, 0f);
					}
					else
					{
						vector7 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (this.m_textInfo.pageInfo[num6].ascender + margin.y + this.m_textInfo.pageInfo[num6].descender - margin.w) / 2f, 0f);
					}
					break;
				...
				case TextAlignmentOptions.BottomJustified:
					if (this.m_overflowMode != TextOverflowModes.Page)
					{
						vector7 = textContainerLocalCorners[0] + new Vector3(0f + margin.x, 0f - num10 + margin.w, 0f);
					}
					else
					{
						vector7 = textContainerLocalCorners[0] + new Vector3(0f + margin.x, 0f - this.m_textInfo.pageInfo[num6].descender + margin.w, 0f);
					}
					break;
				...
				case TextAlignmentOptions.BaselineJustified:
					vector7 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f, 0f);
					break;
				case TextAlignmentOptions.MidlineLeft:
				case TextAlignmentOptions.Midline:
				case TextAlignmentOptions.MidlineRight:
				case TextAlignmentOptions.MidlineJustified:
					vector7 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (this.m_meshExtents.max.y + margin.y + this.m_meshExtents.min.y - margin.w) / 2f, 0f);
					break;
				}
				...
				for (int i = 0; i < this.m_characterCount; i++)
				{
					...
					case TextAlignmentOptions.MidlineLeft:
						if (!this.m_isRightToLeft)
						{
							vector8 = new Vector3(0f + tmp_LineInfo.marginLeft, 0f, 0f);
						}
						else
						{
		...
		public float lineLength
		{
			...
			set
			{
				global::Debug.Log("lineLength set called.", null);
			}
		}
		...
		virtual float UnityEngine.UI.ILayoutElement.get_minWidth()
		{
			return base.minWidth;
		}

		virtual float UnityEngine.UI.ILayoutElement.get_flexibleWidth()
		{
			return base.flexibleWidth;
		}

		virtual float UnityEngine.UI.ILayoutElement.get_minHeight()
		{
			return base.minHeight;
		}

		virtual float UnityEngine.UI.ILayoutElement.get_flexibleHeight()
		{
			return base.flexibleHeight;
		}

		virtual int UnityEngine.UI.ILayoutElement.get_layoutPriority()
		{
