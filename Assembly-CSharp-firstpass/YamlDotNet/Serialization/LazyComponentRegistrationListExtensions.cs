...
namespace YamlDotNet.Serialization
{
	internal static class LazyComponentRegistrationListExtensions
	{
		...
		{
			return registrations.Select<Func<Nothing, TComponent>, TComponent>((Func<Nothing, TComponent> factory) => factory(null)).ToList<TComponent>();
		}
		...
		{
			return registrations.Select<Func<TArgument, TComponent>, TComponent>((Func<TArgument, TComponent> factory) => factory(argument)).ToList<TComponent>();
		}
	}
}
