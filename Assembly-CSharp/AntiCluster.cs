...
public class AntiCluster : KMonoBehaviour
{
	...
	private void OnCellChange(int previous_cell, int new_cell)
	{
		...
		{
			GameObject gameObject = Grid.Objects[previous_cell, 1];
			if (gameObject == base.gameObject)
			{
				Grid.Objects[previous_cell, 1] = null;
			}
		}
		if (Grid.IsValidCell(new_cell))
		{
			GameObject gameObject2 = Grid.Objects[new_cell, 1];
			if (gameObject2 == null)
			{
				Grid.Objects[new_cell, 1] = base.gameObject;
			}
			else
			{
