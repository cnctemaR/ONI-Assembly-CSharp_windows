			...
			if ((float)bodyContainedIn.containedObjects.Count < (float)bodyContainedIn.waterCells.Count * this.maxmimumDensity)
			{
				if (!this.SpawnEgg)
				{
					...
					while (num == -1)
					{
						...
						{
							if (Grid.PosToCell(gameObject) != num3)
							{
								if (Grid.IsSubstantialLiquid(num3, 0.35f) && Grid.IsSubstantialLiquid(Grid.CellDownRight(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellDownLeft(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellUpRight(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellUpLeft(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellAbove(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellBelow(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellLeft(num3), 0.35f) && Grid.IsSubstantialLiquid(Grid.CellRight(num3), 0.35f))
								{
									num = num3;
									break;
								}
