...
public class ConduitBridge : KMonoBehaviour
{
	...
	protected override void OnSpawn()
	{
		...
		Conduit.GetFlowManager(this.type).AddConduitUpdater(new Action<float>(this.ConduitUpdate), 0);
	}
	...
	private void ConduitUpdate(float dt)
	{
		...
		if (contents.mass > 0f)
		{
			float num = flowManager.AddElement(this.outputCell, contents.element, contents.mass, contents.temperature);
			if (num > 0f)
			{
