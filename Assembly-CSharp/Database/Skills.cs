...
namespace Database
{
	public class Skills : ResourceSet<Skill>
	{
			...
			: base("Skills", parent)
		{
			...
			if (DlcManager.IsExpansion1Active())
			{
				this.ThermalSuits = this.AddSkill(new Skill("ThermalSuits", DUPLICANTS.ROLES.THERMAL_SUIT_WEARER.NAME, DUPLICANTS.ROLES.THERMAL_SUIT_WEARER.DESCRIPTION, "EXPANSION1_ID", 1, "hat_role_suits1", "skillbadge_role_suits2", Db.Get().SkillGroups.Suits.Id, new List<SkillPerk> { Db.Get().SkillPerks.IncreaseAthleticsLarge }, new List<string>
				{
					this.Hauling1.Id,
					this.RocketPiloting1.Id
				}));
				this.Suits1 = this.AddSkill(new Skill("Suits1", DUPLICANTS.ROLES.SUIT_EXPERT.NAME, DUPLICANTS.ROLES.SUIT_EXPERT.DESCRIPTION, "EXPANSION1_ID", 2, "hat_role_suits1", "skillbadge_role_suits3", Db.Get().SkillGroups.Suits.Id, new List<SkillPerk>
				{
					Db.Get().SkillPerks.ExosuitExpertise,
					Db.Get().SkillPerks.IncreaseAthleticsMedium
				}, new List<string> { DlcManager.IsExpansion1Active() ? this.ThermalSuits.Id : this.Hauling2.Id }));
			}
			else
			{
				this.Suits1 = this.AddSkill(new Skill("Suits1", DUPLICANTS.ROLES.SUIT_EXPERT.NAME, DUPLICANTS.ROLES.SUIT_EXPERT.DESCRIPTION, "", 2, "hat_role_suits1", "skillbadge_role_suits3", Db.Get().SkillGroups.Suits.Id, new List<SkillPerk>
				{
					Db.Get().SkillPerks.ExosuitExpertise,
					Db.Get().SkillPerks.IncreaseAthleticsMedium
				}, new List<string> { this.Hauling2.Id }));
			}
