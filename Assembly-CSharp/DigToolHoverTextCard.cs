...
public class DigToolHoverTextCard : HoverTextConfiguration
{
	public override void ConfigureHoverScreen()
	{
		using (new KProfiler.Region("ConfigureDigToolHoverScreen", null))
		{
			...
			else
			{
				...
				this.hoverScreenElements.ElementHardnessDescription = instance.AddText("", this.Styles_Values.Property_Unit.Standard, true);
				instance.EndShadowBar();
				this.isConfigured = true;
			}
		}
	...
	public override void UpdateHoverElements(List<KSelectable> selected)
	{
		...
		if (Grid.IsValidCell(num))
		{
			...
			this.hoverScreenElements.ElementHardnessDescription.text = ((!Grid.Element[num].IsSolid) ? "" : (" " + GameUtil.GetHardnessString(Grid.Element[num], true)));
			if (flag)
			{
