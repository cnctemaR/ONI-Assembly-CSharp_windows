...
namespace Generated
{
	public class Border : Path, SymbolicMapElement
	{
		...
		public void ConvertToMap(Chunk world, TerrainCell.SetValuesFunction SetValues, float temperatureMin, float temperatureRange)
		{
			for (int i = 0; i < this.pathElements.Count; i++)
			{
				...
				List<Vector2I> line = Util.GetLine(this.pathElements[i].e0, this.pathElements[i].e1);
				for (int j = 0; j < line.Count; j++)
				{
					int num = Grid.XYToCell(line[j].x, line[j].y);
					if (Grid.IsValidCell(num))
					{
						Element element = ElementLoader.FindElementByHash(Util.WeightedRandom.Choose<WeightedSimHash>(this.element).element);
					...
					for (float num2 = 0.5f; num2 <= this.width; num2 += 1f)
					{
						...
						if (Grid.IsValidCell(num))
						{
							Element element2 = ElementLoader.FindElementByHash(Util.WeightedRandom.Choose<WeightedSimHash>(this.element).element);
							...
							SetValues(num, element2, defaultValues2);
						}
						Vector2 vector4 = line[j] - normalized * num2;
						num = Grid.XYToCell((int)vector4.x, (int)vector4.y);
						if (Grid.IsValidCell(num))
						{
							Element element3 = ElementLoader.FindElementByHash(Util.WeightedRandom.Choose<WeightedSimHash>(this.element).element);
							...
							SetValues(num, element3, defaultValues3);
						}
					}
				}
			}
