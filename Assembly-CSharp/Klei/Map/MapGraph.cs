...
namespace Klei.Map
{
	...
	public class MapGraph : Graph
	{
		...
		protected override void DrawNodes()
		{
			...
			for (int i = 0; i < this.cellList.Count; i++)
			{
				Node node = this.cellList[i];
				DebugExtension.DebugPoint(node.position, Color.white, 1f, 0f, true);
			}
			for (int j = 0; j < this.cornerList.Count; j++)
			{
				Node node2 = this.cornerList[j];
				DebugExtension.DebugPoint(node2.position, Color.yellow, 1f, 0f, true);
			}
		}

		protected override void DrawArcs()
		{
			base.DrawArcs();
			for (int i = 0; i < this.edgeList.Count; i++)
			{
