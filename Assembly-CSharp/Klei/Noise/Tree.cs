...
namespace Klei.Noise
{
	public class Tree : YamlIO<Tree>
	{
		...
		private PrimitiveModule GetPrimitiveModule(string name)
		{
			if (!this.primitives.ContainsKey(name))
			{
				return null;
			}
			return (PrimitiveModule)this.primitives[name].CreateModule();
		}

		private FilterModule GetFilterModule(string name)
		{
			if (!this.filters.ContainsKey(name))
			{
				return null;
			}
			return (FilterModule)this.filters[name].CreateModule();
		}
		...
		public IModule3D BuildFinalModule()
		{
			...
			{
				this.primitiveLookup.Add(keyValuePair.Key, keyValuePair.Value.CreateModule());
			}
