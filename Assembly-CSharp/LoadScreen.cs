...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 273690U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 273690U);
				this.loadButton.isInteractable = false;
	...
	private void DoLoad()
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 273690U)
		{
			text = header.buildVersion.ToString();
			text2 = 273690U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
