...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 217311U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 217311U);
	...
	private void DoLoad()
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 217311U)
		{
			text = header.buildVersion.ToString();
			text2 = 217311U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
