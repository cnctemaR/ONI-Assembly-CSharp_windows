...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 247173U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 247173U);
				this.loadButton.isInteractable = false;
	...
	private void DoLoad()
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 247173U)
		{
			text = header.buildVersion.ToString();
			text2 = 247173U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
