...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 381818U;
	}
	...
	private void SetSelectedGame(string filename, string savename)
	{
		...
		try
		{
			...
			{
				this.InfoText.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 381818U);
				this.loadButton.isInteractable = false;
	...
	private static void DoLoad(string filename)
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 381818U)
		{
			text = header.buildVersion.ToString();
			text2 = 381818U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
