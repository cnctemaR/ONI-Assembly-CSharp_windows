...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 230365U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 230365U);
				this.loadButton.isInteractable = false;
	...
	private void DoLoad()
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 230365U)
		{
			text = header.buildVersion.ToString();
			text2 = 230365U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
