...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 285039U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 285039U);
				this.loadButton.isInteractable = false;
	...
	private static void DoLoad(string filename)
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 285039U)
		{
			text = header.buildVersion.ToString();
			text2 = 285039U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
