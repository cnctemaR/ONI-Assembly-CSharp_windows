...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 280216U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 280216U);
				this.loadButton.isInteractable = false;
	...
	private void DoLoad()
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 280216U)
		{
			text = header.buildVersion.ToString();
			text2 = 280216U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
