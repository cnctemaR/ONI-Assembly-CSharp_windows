...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 242561U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		else
		{
			...
			try
			{
				...
				{
					this.saveDetails.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 242561U);
					this.loadButton.isInteractable = false;
	...
	private void DoLoad()
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 242561U)
		{
			text = header.buildVersion.ToString();
			text2 = 242561U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
