...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 357226U;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		try
		{
			...
			{
				this.InfoText.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 357226U);
				this.loadButton.isInteractable = false;
	...
	private static void DoLoad(string filename)
	{
		...
		string text = null;
		string text2 = null;
		if (header.buildVersion > 357226U)
		{
			text = header.buildVersion.ToString();
			text2 = 357226U.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
