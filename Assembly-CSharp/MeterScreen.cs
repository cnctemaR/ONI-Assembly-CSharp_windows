...
public class MeterScreen : KScreen
{
	...
	private float GetWorstImmunity()
	{
		...
		else
		{
			...
			float num2 = Db.Get().Amounts.ImmuneLevel.Lookup(liveMinionIdentities[0]).value;
			for (int i = 1; i < liveMinionIdentities.Count; i++)
			{
				num2 = Mathf.Min(Db.Get().Amounts.ImmuneLevel.Lookup(liveMinionIdentities[i]).value, num2);
			}
			num = num2;
		}
		return num;
