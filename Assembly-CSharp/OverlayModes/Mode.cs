...
namespace OverlayModes
{
	public abstract class Mode
	{
		...
		protected static void ResetDisplayValues<T>(ICollection<T> targets) where T : KMonoBehaviour
		{
		...
		protected static void RemoveOffscreenTargets<T>(ICollection<T> targets, Vector2I min, Vector2I max) where T : KMonoBehaviour
		{
		...
		protected void AddTargetIfVisible<T>(T cmp, Vector2I vis_min, Vector2I vis_max, ICollection<T> targets, int layer) where T : KMonoBehaviour
		{
			if (cmp == null)
			{
				return;
			}
			Vector2I vector2I = Grid.PosToXY(cmp.transform.position);
			if (vis_min <= vector2I && vector2I <= vis_max)
			{
				int num = Grid.XYToCell(vector2I.x, vector2I.y);
				if (Grid.Visible[num] > 128 && !targets.Contains(cmp))
				{
					KBatchedAnimController component = cmp.GetComponent<KBatchedAnimController>();
					if (component != null)
					{
