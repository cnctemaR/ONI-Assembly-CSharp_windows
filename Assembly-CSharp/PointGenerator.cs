...
{
	public static List<Vector2> GetRandomPoints(Polygon boundingArea, float density, float avoidRadius, List<Vector2> avoidPoints, PointGenerator.SampleBehaviour behaviour, bool testInsideBounds, bool doShuffle = true, bool testAvoidPoints = true)
	{
		...
		case PointGenerator.SampleBehaviour.UniformCircle:
		{
			...
			float num21 = WorldGen.RandomValue();
		...
		case PointGenerator.SampleBehaviour.PoissonDisk:
			list = UniformPoissonDiskSampler.SampleRectangle(min, max, density, num7);
			goto IL_03A0;
		}
		for (float num26 = -num4 + avoidRadius * 0.3f + WorldGen.RandomValue() * 2f; num26 < num4 - (avoidRadius * 0.3f + WorldGen.RandomValue() * 2f); num26 += density + WorldGen.RandomValue())
		{
			for (float num27 = -num3 + avoidRadius * 0.3f + WorldGen.RandomValue() * 2f + WorldGen.RandomValue() * 2f; num27 < num3 - (avoidRadius * 0.3f + WorldGen.RandomValue() * 2f); num27 += density + WorldGen.RandomValue())
			{
				list.Add(boundingArea.Centroid() + new Vector2(num27, num26 + WorldGen.RandomValue() - 0.5f));
			}
		}
		IL_03A0:
		...
		if (doShuffle)
		{
			global::System.Random random = WorldGen.RandomSource();
			if (random != null)
			{
				list2.ShuffleSeeded<Vector2>(random);
			}
			else
			{
	...
	public static List<Vector2> GetSpaceFillingRandom(Rect boundingArea, float density)
	{
		List<Vector2> filliedRectangle = PointGenerator.GetFilliedRectangle(boundingArea, density);
		filliedRectangle.ShuffleSeeded<Vector2>(WorldGen.RandomSource());
