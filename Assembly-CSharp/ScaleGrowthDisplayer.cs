...
public class ScaleGrowthDisplayer : AsPercentAmountDisplayer
{
	...
	public override string GetDescription(Amount master, AmountInstance instance)
	{
		...
		string text = (CREATURES.STATS.SCALEGROWTH.GET_DISPLAYED_NAME().ContainsKey(tag) ? CREATURES.STATS.SCALEGROWTH.GET_DISPLAYED_NAME()[tag] : master.Name);
	...
	{
		Tag tag = instance.gameObject.PrefabID();
		string text = (CREATURES.STATS.SCALEGROWTH.GET_TOOLTIP_PREFIX().ContainsKey(tag) ? CREATURES.STATS.SCALEGROWTH.GET_TOOLTIP_PREFIX()[tag] : "");
