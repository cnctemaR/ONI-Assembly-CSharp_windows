...
public class SplashTransitionLayer : TransitionDriver.OverrideLayer
{
	...
	private void RefreshSplashes(Navigator navigator)
	{
		...
		{
			this.lastSplashTime = Time.time;
			KBatchedAnimController kbatchedAnimController = FXHelpers.CreateEffect("splash_step", null, false, Grid.SceneLayer.Front);
			kbatchedAnimController.transform.SetPosition(navigator.transform.position + new Vector3(0f, 0.75f, -0.1f));
