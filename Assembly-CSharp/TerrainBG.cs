...
public class TerrainBG : KMonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		this.backgroundMaterial.renderQueue = 2999;
		float num = 0f;
		for (int i = 0; i < Lighting.Instance.Settings.BackgroundLayers; i++)
		{
			if (i >= Lighting.Instance.Settings.BackgroundLayers - 1)
			{
				float num2 = (float)i / (float)(Lighting.Instance.Settings.BackgroundLayers - 1);
				float num3 = Mathf.Lerp(1f, Lighting.Instance.Settings.BackgroundDarkening, num2);
				float num4 = num + Lighting.Instance.Settings.BackgroundLayerParallax * (float)i;
				float num5 = Mathf.Lerp(1f, Lighting.Instance.Settings.BackgroundUVScale, num2);
				float num6 = 1f;
				if (i == Lighting.Instance.Settings.BackgroundLayers - 1)
				{
					num6 = 0f;
				...
				materialPropertyBlock.SetVector("_Parallax", new Vector4(num4, 0f, 0f, 0f));
				materialPropertyBlock.SetVector("_BackWallParameters", new Vector4(num3, Lighting.Instance.Settings.BackgroundClip, num5, num6));
