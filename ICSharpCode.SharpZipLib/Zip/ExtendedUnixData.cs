			...
			using (MemoryStream memoryStream = new MemoryStream(data, index, count, false))
			{
				using (ZipHelperStream zipHelperStream = new ZipHelperStream(memoryStream))
				{
					...
					if ((byte)(this._flags & ExtendedUnixData.Flags.ModificationTime) != 0 && count >= 5)
					{
						...
						this._modificationTime = (dateTime.ToUniversalTime() + new TimeSpan(0, 0, 0, num, 0)).ToLocalTime();
					}
					if ((byte)(this._flags & ExtendedUnixData.Flags.AccessTime) != 0)
					{
						...
						this._lastAccessTime = (dateTime2.ToUniversalTime() + new TimeSpan(0, 0, 0, num2, 0)).ToLocalTime();
					}
					if ((byte)(this._flags & ExtendedUnixData.Flags.CreateTime) != 0)
					{
						...
						this._createTime = (dateTime3.ToUniversalTime() + new TimeSpan(0, 0, 0, num3, 0)).ToLocalTime();
					}
				}
			}
		}
		...
		public byte[] GetData()
		{
			...
			using (MemoryStream memoryStream = new MemoryStream())
			{
				using (ZipHelperStream zipHelperStream = new ZipHelperStream(memoryStream))
				{
					...
					{
						DateTime dateTime = this._modificationTime.ToUniversalTime();
						DateTime dateTime2 = new DateTime(1970, 1, 1, 0, 0, 0);
						int num = (int)(dateTime - dateTime2.ToUniversalTime()).TotalSeconds;
						zipHelperStream.WriteLEInt(num);
					}
					if ((byte)(this._flags & ExtendedUnixData.Flags.AccessTime) != 0)
					{
						DateTime dateTime3 = this._lastAccessTime.ToUniversalTime();
						DateTime dateTime4 = new DateTime(1970, 1, 1, 0, 0, 0);
						int num2 = (int)(dateTime3 - dateTime4.ToUniversalTime()).TotalSeconds;
						zipHelperStream.WriteLEInt(num2);
					}
					if ((byte)(this._flags & ExtendedUnixData.Flags.CreateTime) != 0)
					{
						DateTime dateTime5 = this._createTime.ToUniversalTime();
						DateTime dateTime6 = new DateTime(1970, 1, 1, 0, 0, 0);
						int num3 = (int)(dateTime5 - dateTime6.ToUniversalTime()).TotalSeconds;
