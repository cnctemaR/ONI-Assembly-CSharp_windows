	...
	public abstract class JsonReader : IDisposable
	{
		...
		public virtual string Path
		{
			get
			{
				...
				IEnumerable<JsonPosition> enumerable = ((!flag) ? this._stack : this._stack.Concat<JsonPosition>(new JsonPosition[] { this._currentPosition }));
				return JsonPosition.BuildPath(enumerable);
			}
		}
			...
			while (this.ReadInternal())
			{
				...
				if (jsonToken != JsonToken.Comment)
				{
					...
					else
					{
						if (jsonToken == JsonToken.StartArray)
						{
							...
							while (this.ReadInternal())
							{
								...
								case JsonToken.Integer:
									...
									continue;
								default:
									if (jsonToken2 == JsonToken.EndArray)
									{
								...
								throw JsonReaderException.Create(this, "Unexpected token when reading bytes: {0}.".FormatWith(CultureInfo.InvariantCulture, jsonToken));
							}
						...
						throw JsonReaderException.Create(this, "Error reading bytes. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture, jsonToken));
					}
				}
			}
			this.SetToken(JsonToken.None);
