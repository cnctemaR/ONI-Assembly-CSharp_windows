...
namespace Newtonsoft.Json.Utilities
{
	internal static class EnumUtils
	{
		...
		public static IList<T> GetFlagsValues<T>(T value) where T : struct
		{
			...
			if (list.Count == 0 && namesAndValues.SingleOrDefault<EnumValue<ulong>>((EnumValue<ulong> v) => v.Value == 0UL) != null)
			{
				list.Add(default(T));
			}
			return list;
		...
		public static object ParseEnumName(string enumText, bool isNullable, Type t)
		{
			...
			if (enumText.IndexOf(',') != -1)
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					array[i] = EnumUtils.ResolvedEnumName(bidirectionalDictionary, text);
				}
				text2 = string.Join(", ", array);
			}
			else
			{
				text2 = EnumUtils.ResolvedEnumName(bidirectionalDictionary, enumText);
			}
