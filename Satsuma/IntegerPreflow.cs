...
namespace Satsuma
{
	public sealed class IntegerPreflow : IFlow<long>
	{
		...
		private void Run()
		{
					...
					if (!(node5 == node6))
					{
						...
						if (node5 == node4)
						{
							if (num6 != num7)
							{
								...
								else
								{
									...
									Node node8;
									(dictionary2 = this.excess)[node8 = node6] = dictionary2[node8] + num10;
						...
						else if (num6 != num8)
						{
							...
							else
							{
								...
								Node node9;
								(dictionary3 = this.excess)[node9 = node5] = dictionary3[node9] + num12;
			...
			{
				Node node10 = this.Graph.U(arc3);
				Node node11 = this.Graph.V(arc3);
				long num13;
				if (!(node10 == node11) && this.flow.TryGetValue(arc3, out num13))
				{
					if (node10 == this.Source)
					{
						this.FlowSize += num13;
					}
					else
