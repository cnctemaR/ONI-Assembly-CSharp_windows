...
namespace System.Drawing
{
	internal sealed class ComIStreamMarshaler : ICustomMarshaler
	{
		...
		private sealed class ManagedToNativeWrapper
		{
			private ManagedToNativeWrapper(IStream managedInterface)
			{
				...
				try
				{
					...
					Marshal.StructureToPtr(streamInterface, this.comInterface, false);
				}
				catch
				{
					this.Dispose();
			...
			internal static void ReleaseInterface(IntPtr comInterface)
			{
				if (comInterface != IntPtr.Zero)
				{
					...
					{
						ComIStreamMarshaler.ManagedToNativeWrapper.ReleaseSlot releaseSlot = (ComIStreamMarshaler.ManagedToNativeWrapper.ReleaseSlot)Marshal.PtrToStructure((IntPtr)((long)intPtr + (long)(IntPtr.Size * 2)), typeof(ComIStreamMarshaler.ManagedToNativeWrapper.ReleaseSlot));
			...
			private static int Release(IntPtr @this)
			{
				...
				try
				{
					...
					lock (managedToNativeWrapper)
					{
						if (@object.refCount != 0 && --@object.refCount == 0)
						{
							@object.Dispose();
						}
						num = @object.refCount;
			...
			private static readonly MethodInfo exceptionGetHResult = typeof(Exception).GetProperty("HResult", BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.GetProperty | BindingFlags.ExactBinding, null, typeof(int), new Type[0], null).GetGetMethod(true);
		...
		private delegate int ReadDelegate(IntPtr @this, [MarshalAs(UnmanagedType.LPArray, SizeConst = 0, SizeParamIndex = 2)] [Out] byte[] pv, int cb, IntPtr pcbRead);

		private delegate int WriteDelegate(IntPtr @this, [MarshalAs(UnmanagedType.LPArray, SizeConst = 0, SizeParamIndex = 2)] byte[] pv, int cb, IntPtr pcbWritten);
