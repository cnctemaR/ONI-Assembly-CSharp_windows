...
namespace System.Drawing.Printing
{
	public class MarginsConverter : ExpandableObjectConverter
	{
		...
		public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
		{
			...
			string text = "( |\\t)*";
			text = text + ";" + text;
			string text2 = string.Concat(new string[] { "(?<left>\\d+)", text, "(?<right>\\d+)", text, "(?<top>\\d+)", text, "(?<bottom>\\d+)" });
			Match match = new Regex(text2).Match(value as string);
			if (!match.Success)
			{
		...
		public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
		{
			if (destinationType == typeof(string) && value is Margins)
			{
				...
				string text = "{0}; {1}; {2}; {3}";
				return string.Format(text, new object[] { margins.Left, margins.Right, margins.Top, margins.Bottom });
			}
