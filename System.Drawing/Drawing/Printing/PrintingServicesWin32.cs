...
namespace System.Drawing.Printing
{
	internal class PrintingServicesWin32 : PrintingServices
	{
		...
		internal override void LoadPrinterSettings(string printer, PrinterSettings settings)
		{
			...
			intPtr2 = PrintingServicesWin32.Win32CreateIC(null, printer, null, IntPtr.Zero);
			...
			try
			{
				...
				if (num2 != -1)
				{
					...
					for (int i = 0; i < num2; i++)
					{
						...
						settings.PaperSources.Add(new PaperSource(text, paperSourceKind));
		...
		{
			int num = PrintingServicesWin32.Win32StartPage(gr.Hdc);
			return num > 0;
		}

		internal static bool EndPage(GraphicsPrinter gr)
		{
			int num = PrintingServicesWin32.Win32EndPage(gr.Hdc);
			return num > 0;
		}
