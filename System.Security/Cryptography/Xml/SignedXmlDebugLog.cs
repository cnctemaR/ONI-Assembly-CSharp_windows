	...
	internal static class SignedXmlDebugLog
	{
		...
		internal static void LogSigningReference(SignedXml signedXml, Reference reference)
		{
			if (SignedXmlDebugLog.VerboseLoggingEnabled)
			{
				...
				string text2 = string.Format(CultureInfo.InvariantCulture, "Hashing reference {0}, Uri \"{1}\", Id \"{2}\", Type \"{3}\" with hash algorithm \"{4}\" ({5}).", new object[]
				{
				...
				SignedXmlDebugLog.WriteLine(signedXml, TraceEventType.Verbose, SignedXmlDebugLog.SignedXmlDebugEvent.SigningReference, text2);
			}
		}
		...
		internal static void LogVerifyReferenceHash(SignedXml signedXml, Reference reference, byte[] actualHash, byte[] expectedHash)
		{
			if (SignedXmlDebugLog.VerboseLoggingEnabled)
			{
				HashAlgorithm hashAlgorithm = CryptoHelpers.CreateFromName<HashAlgorithm>(reference.DigestMethod);
				string text = ((hashAlgorithm == null) ? "null" : hashAlgorithm.GetType().Name);
				string text2 = string.Format(CultureInfo.InvariantCulture, "Reference {0} hashed with \"{1}\" ({2}) has hash value {3}, expected hash value {4}.", new object[]
				{
					SignedXmlDebugLog.GetObjectId(reference),
					reference.DigestMethod,
					text,
				...
				SignedXmlDebugLog.WriteLine(signedXml, TraceEventType.Verbose, SignedXmlDebugLog.SignedXmlDebugEvent.VerifyReference, text2);
			}
		}
		...
		internal static void LogSignedXmlRecursionLimit(SignedXml signedXml, Reference reference)
		{
			if (SignedXmlDebugLog.InformationLoggingEnabled)
			{
				HashAlgorithm hashAlgorithm = CryptoHelpers.CreateFromName<HashAlgorithm>(reference.DigestMethod);
				string text = ((hashAlgorithm == null) ? "null" : hashAlgorithm.GetType().Name);
				string text2 = string.Format(CultureInfo.InvariantCulture, "Signed xml recursion limit hit while trying to decrypt the key. Reference {0} hashed with \"{1}\" and ({2}).", SignedXmlDebugLog.GetObjectId(reference), reference.DigestMethod, text);
				SignedXmlDebugLog.WriteLine(signedXml, TraceEventType.Information, SignedXmlDebugLog.SignedXmlDebugEvent.VerifySignedInfo, text2);
			}
		}
