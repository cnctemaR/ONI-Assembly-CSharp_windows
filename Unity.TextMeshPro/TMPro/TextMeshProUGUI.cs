...
namespace TMPro
{
	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		internal override int SetArraySizes(TMP_Text.TextProcessingElement[] textProcessingArray)
		{
			...
			{
				TMP_Text tmp_Text = this.m_linkedTextComponent;
				while (tmp_Text != null)
				{
		...
		protected virtual void GenerateTextMesh()
		{
			...
			bool flag24 = false;
			int num89 = 0;
			bool flag25 = !(this.m_canvas.worldCamera == null);
			float num90 = (this.m_previousLossyScaleY = base.transform.lossyScale.y);
			...
			while (i < this.m_characterCount)
			{
				...
				bool flag26 = char.IsWhiteSpace(character);
				...
				if (isVisible)
				{
					...
					else
					{
						...
						num91 = characterInfo[i].scale * (1f - this.m_charWidthAdjDelta) * this.m_characterHorizontalScale;
						...
						case RenderMode.ScreenSpaceOverlay:
							num91 *= Mathf.Abs(num90) / scaleFactor;
							break;
						case RenderMode.ScreenSpaceCamera:
							num91 *= (flag25 ? Mathf.Abs(num90) : 1f);
							break;
						case RenderMode.WorldSpace:
							num91 *= Mathf.Abs(num90);
							break;
						}
				...
				else if ((flag24 || (i == 0 && (!char.IsPunctuation(character) || flag26 || character == '\u200b' || i == this.m_characterCount - 1))) && (i <= 0 || i >= characterInfo.Length - 1 || i >= this.m_characterCount || (character != '\'' && character != '’') || !char.IsLetterOrDigit(characterInfo[i - 1].character) || !char.IsLetterOrDigit(characterInfo[i + 1].character)))
				{
				...
				bool flag28 = (this.m_textInfo.characterInfo[i].style & FontStyles.Strikethrough) == FontStyles.Strikethrough;
				...
				if (flag28)
				{
					...
					if (!flag2 && flag29 && i <= tmp_LineInfo.lastVisibleCharacterIndex && character != '\n' && character != '\v' && character != '\r' && (i != tmp_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character)))
					{
						flag2 = true;
				...
				if ((this.m_textInfo.characterInfo[i].style & FontStyles.Highlight) == FontStyles.Highlight)
				{
					...
					if (!flag3 && flag30 && i <= tmp_LineInfo.lastVisibleCharacterIndex && character != '\n' && character != '\v' && character != '\r' && (i != tmp_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character)))
					{
						flag3 = true;
				...
				bool flag32 = (alignment & HorizontalAlignmentOptions.Flush) == HorizontalAlignmentOptions.Flush;
				if ((!char.IsControl(character2) && lineNumber5 < this.m_lineNumber) || flag32 || tmp_LineInfo.maxAdvance > tmp_LineInfo.width)
				{
			...
			if (this.m_renderMode == TextRenderFlags.Render && this.IsActive())
			{
				...
				for (int j = 1; j < this.m_textInfo.materialCount; j++)
				{
					...
					TMP_SubMeshUI tmp_SubMeshUI = this.m_subTextObjects[j];
					if (!(tmp_SubMeshUI == null))
					{
						...
						tmp_SubMeshUI.mesh.vertices = this.m_textInfo.meshInfo[j].vertices;
						tmp_SubMeshUI.mesh.SetUVs(0, this.m_textInfo.meshInfo[j].uvs0);
						tmp_SubMeshUI.mesh.uv2 = this.m_textInfo.meshInfo[j].uvs2;
						tmp_SubMeshUI.mesh.colors32 = this.m_textInfo.meshInfo[j].colors32;
						tmp_SubMeshUI.mesh.RecalculateBounds();
						tmp_SubMeshUI.canvasRenderer.SetMesh(tmp_SubMeshUI.mesh);
						tmp_SubMeshUI.canvasRenderer.SetColor(color4);
						tmp_SubMeshUI.canvasRenderer.cullTransparentMesh = cullTransparentMesh;
						tmp_SubMeshUI.raycastTarget = this.raycastTarget;
		...
		internal override Rect GetCanvasSpaceClippingRect()
		{
			...
			Vector2 vector = transform.InverseTransformPoint(this.m_rectTransform.position);
			Vector2 vector2 = transform.lossyScale;
			Vector2 vector3 = this.m_rectTransform.lossyScale / vector2;
