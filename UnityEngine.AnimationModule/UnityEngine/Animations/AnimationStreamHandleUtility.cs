...
namespace UnityEngine.Animations
{
	...
	public static class AnimationStreamHandleUtility
	{
		...
		{
			stream.CheckIsValid();
			int num = AnimationSceneHandleUtility.ValidateAndGetArrayCount<PropertyStreamHandle, EntityId>(ref stream, handles, buffer);
			bool flag = num == 0;
			if (!flag)
			{
				AnimationStreamHandleUtility.WriteStreamEntityIdsInternal(ref stream, handles.GetUnsafePtr<PropertyStreamHandle>(), buffer.GetUnsafePtr<EntityId>(), num, useMask);
			}
		}
		...
		public static void ReadEntityIds(AnimationStream stream, NativeArray<PropertyStreamHandle> handles, NativeArray<EntityId> buffer)
		{
			...
			if (!flag)
			{
				AnimationStreamHandleUtility.ReadStreamEntityIdsInternal(ref stream, handles.GetUnsafePtr<PropertyStreamHandle>(), buffer.GetUnsafePtr<EntityId>(), num);
			}
		}
