			...
			foreach (PeerInfoMessage peerInfoMessage in this.m_Peers)
			{
				if (peerInfoMessage.connectionId == peerAuthorityMessage.connectionId)
				{
					...
					if (peerAuthorityMessage.authorityState)
					{
						...
						PeerInfoPlayer peerInfoPlayer2 = default(PeerInfoPlayer);
						peerInfoPlayer2.netId = peerAuthorityMessage.netId;
						peerInfoPlayer2.playerControllerId = -1;
						peerInfoMessage.playerIds = new List<PeerInfoPlayer>(peerInfoMessage.playerIds) { peerInfoPlayer2 }.ToArray();
					}
					else
					{
