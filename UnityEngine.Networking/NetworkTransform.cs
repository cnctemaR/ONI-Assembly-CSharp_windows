...
namespace UnityEngine.Networking
{
	...
	public class NetworkTransform : NetworkBehaviour
	{
		...
		private bool CheckVelocityChanged()
		{
			...
			if (transformSyncMode != NetworkTransform.TransformSyncMode.SyncRigidbody2D)
			{
				flag = transformSyncMode == NetworkTransform.TransformSyncMode.SyncRigidbody3D && (this.m_RigidBody3D && this.m_VelocityThreshold > 0f) && Mathf.Abs(this.m_RigidBody3D.velocity.sqrMagnitude - this.m_PrevVelocity) >= this.m_VelocityThreshold;
			}
			else
			{
				flag = this.m_RigidBody2D && this.m_VelocityThreshold > 0f && Mathf.Abs(this.m_RigidBody2D.velocity.sqrMagnitude - this.m_PrevVelocity) >= this.m_VelocityThreshold;
			}
			return flag;
		}
