...
namespace UnityEngine.UIElements.UIR.Implementation
{
	internal static class RenderEvents
	{
		...
		internal static void ProcessOnClippingChanged(RenderChain renderChain, VisualElement ve, uint dirtyID, UIRenderDevice device, ref ChainBuilderStats stats)
		{
			...
			RenderEvents.DepthFirstOnClippingChanged(renderChain, ve.hierarchy.parent, ve, dirtyID, flag, true, false, false, false, device, ref stats);
		}
		...
		internal static void ProcessOnOpacityChanged(RenderChain renderChain, VisualElement ve, uint dirtyID, ref ChainBuilderStats stats)
		{
			...
			RenderEvents.DepthFirstOnOpacityChanged(renderChain, (ve.hierarchy.parent != null) ? ve.hierarchy.parent.renderChainData.compositeOpacity : 1f, ve, dirtyID, ref stats);
		}
		...
		internal static void ProcessOnTransformOrSizeChanged(RenderChain renderChain, VisualElement ve, uint dirtyID, UIRenderDevice device, ref ChainBuilderStats stats)
		{
			...
			RenderEvents.DepthFirstOnTransformOrSizeChanged(renderChain, ve.hierarchy.parent, ve, dirtyID, device, false, false, ref stats);
		}
		...
		internal static void ProcessOnVisualsChanged(RenderChain renderChain, VisualElement ve, uint dirtyID, ref ChainBuilderStats stats)
		{
			...
			RenderEvents.DepthFirstOnVisualsChanged(renderChain, ve, dirtyID, parent != null && (parent.renderChainData.isHierarchyHidden || RenderEvents.IsElementHierarchyHidden(parent)), flag, ref stats);
		}
		...
		private static void DepthFirstOnClippingChanged(RenderChain renderChain, VisualElement parent, VisualElement ve, uint dirtyID, bool hierarchical, bool isRootOfChange, bool isPendingHierarchicalRepaint, bool inheritedClipRectIDChanged, bool inheritedStencilClippedChanged, UIRenderDevice device, ref ChainBuilderStats stats)
		{
			...
			if (!flag2)
			{
				...
				ClipMethod clipMethod2 = (flag5 ? RenderEvents.DetermineSelfClipMethod(ve) : clipMethod);
				bool flag11 = false;
				bool flag12 = flag4;
				if (flag12)
				{
		...
		private static void DepthFirstOnOpacityChanged(RenderChain renderChain, float parentCompositeOpacity, VisualElement ve, uint dirtyID, ref ChainBuilderStats stats)
		{
			bool flag = dirtyID == ve.renderChainData.dirtyID;
			if (!flag)
			{
				...
				bool flag2 = num == ve.renderChainData.compositeOpacity;
				if (!flag2)
				{
					bool flag3 = ve.renderChainData.compositeOpacity < Mathf.Epsilon && num >= Mathf.Epsilon;
					ve.renderChainData.compositeOpacity = num;
					bool flag4 = false;
					bool flag5 = num != parentCompositeOpacity;
					if (flag5)
					{
						bool flag6 = ve.renderChainData.opacityID.owned == 0;
						if (flag6)
						{
							flag4 = true;
							ve.renderChainData.opacityID = renderChain.shaderInfoAllocator.AllocOpacity();
						}
						bool flag7 = ve.renderChainData.opacityID.IsValid();
						if (flag7)
						{
					...
					{
						bool flag8 = ve.renderChainData.opacityID.owned == 0;
						if (flag8)
						{
							bool flag9 = ve.hierarchy.parent != null && !ve.renderChainData.opacityID.Equals(ve.hierarchy.parent.renderChainData.opacityID);
							if (flag9)
							{
								flag4 = true;
						...
						{
							bool flag10 = ve.renderChainData.opacityID.IsValid();
							if (flag10)
							{
								renderChain.shaderInfoAllocator.SetOpacityValue(ve.renderChainData.opacityID, num);
							}
					...
					{
						bool flag12 = flag4 && (ve.renderChainData.dirtiedValues & RenderDataDirtyTypes.Visuals) == RenderDataDirtyTypes.None;
						if (flag12)
						{
					...
					for (int i = 0; i < childCount; i++)
					{
						RenderEvents.DepthFirstOnOpacityChanged(renderChain, num, ve.hierarchy[i], dirtyID, ref stats);
					}
				}
			}
		}
			...
			else
			{
				...
				if (needsClosing)
				{
					...
					if (popViewMatrix)
					{
						...
						RenderEvents.InjectClosingCommandInBetween(renderChainCommand10, ref renderChainCommand8, ref renderChainCommand9);
					}
					...
					if (popScissorClip)
					{
						...
						RenderEvents.InjectClosingCommandInBetween(renderChainCommand11, ref renderChainCommand8, ref renderChainCommand9);
					}
				}
		...
		private static RenderChainCommand InjectClosingMeshDrawCommand(RenderChain renderChain, VisualElement ve, ref RenderChainCommand cmdPrev, ref RenderChainCommand cmdNext, MeshHandle mesh, int indexCount, int indexOffset, Material material, Texture custom, Texture font)
		{
			...
			renderChainCommand.indexCount = indexCount;
			renderChainCommand.owner = ve;
			RenderEvents.InjectClosingCommandInBetween(renderChainCommand, ref cmdPrev, ref cmdNext);
			return renderChainCommand;
		}
