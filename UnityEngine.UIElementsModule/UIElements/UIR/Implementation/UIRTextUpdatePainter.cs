...
namespace UnityEngine.UIElements.UIR.Implementation
{
	internal class UIRTextUpdatePainter : IStylePainter, IDisposable
	{
		...
		public void Begin(VisualElement ve, UIRenderDevice device)
		{
			...
			NativeSlice<Vertex> nativeSlice = ve.renderChainData.data.allocPage.vertices.cpuData.Slice<Vertex>((int)allocVerts.start, (int)allocVerts.size);
		...
		public MeshWriteData DrawMesh(int vertexCount, int indexCount, Texture texture, Material material, MeshGenerationContext.MeshFlags flags)
		{
			...
			{
				m_Vertices = this.m_DudVerts.Slice<Vertex>(0, vertexCount),
				m_Indices = this.m_DudIndices.Slice<ushort>(0, indexCount)
			};
		}
		...
		public void DrawText(MeshGenerationContextUtils.TextParams textParams, TextHandle handle, float pixelsPerPoint)
		{
			...
			TextNativeSettings textNativeSettings = new TextNativeSettings
			{
			...
			bool flag = textNativeSettings.font == null;
			if (!flag)
			{
				using (NativeArray<TextVertex> vertices = TextNative.GetVertices(textNativeSettings))
				{
					...
					MeshBuilder.UpdateText(vertices, offset, this.m_CurrentElement.renderChainData.verticesSpace, this.m_XFormClipPages, this.m_IDsFlags, this.m_OpacityPagesSettingsIndex, this.m_MeshDataVerts.Slice<Vertex>(renderChainTextEntry.firstVertex, renderChainTextEntry.vertexCount));
