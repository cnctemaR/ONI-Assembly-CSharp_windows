...
namespace UnityEngine.UIElements
{
	public class VisualElement : Focusable, ITransform, ITransitionAnimations, IExperimentalFeatures, IVisualElementScheduler, IResolvedStyle
	{
		...
		internal InheritedStylesData inheritedStyle
		{
			get
			{
				return this.m_InheritedStylesData;
			}
			set
			{
				bool flag = this.m_InheritedStylesData != value;
				if (flag)
				{
					this.m_InheritedStylesData = value;
		...
		{
			bool flag = state == ((this.pseudoStates & PseudoStates.Disabled) != PseudoStates.Disabled);
			bool flag2;
			if (flag)
			{
				flag2 = false;
			...
			{
				bool flag3 = state && this.enabledSelf && (this.parent == null || this.parent.enabledInHierarchy);
				if (flag3)
				{
		...
		{
			bool flag = this.SetEnabledFromHierarchy(value);
			if (flag)
			{
				int count = this.m_Children.Count;
				for (int i = 0; i < count; i++)
		...
		private void FinalizeLayout()
		{
			...
			if (hasInlineStyle)
			{
				this.specifiedStyle.SyncWithLayout(this.yogaNode);
			}
			else
			{
				this.yogaNode.CopyStyle(this.specifiedStyle.yogaNode);
			}
		}
		...
		internal void SetInlineStyles(VisualElementStylesData inlineStyleData)
		{
			...
			inlineStyleData.Apply(this.m_Style, StylePropertyApplyMode.CopyIfEqualOrGreaterSpecificity);
			this.m_Style = inlineStyleData;
		}
		...
		internal void SetSharedStyles(VisualElementStylesData sharedStyle)
		{
			...
			if (!flag)
			{
				StyleInt overflow = this.m_Style.overflow;
		...
		{
			return this.computedStyle.overflow.value > Overflow.Visible;
		}
		...
		public IStyle style
		{
			get
			{
				bool flag = this.m_InlineStyleAccess == null;
				if (flag)
				{
					this.m_InlineStyleAccess = new InlineStyleAccess(this);
				}
				return this.m_InlineStyleAccess;
			}
		}
		...
		Color IResolvedStyle.borderColor
		{
			get
			{
