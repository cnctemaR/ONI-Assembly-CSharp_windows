...
namespace UnityEngine.UIElements
{
	...
	public struct Angle : IEquatable<Angle>
	{
		...
		internal unsafe static bool TryParseString(string str, out Angle angle)
		{
			angle = default(Angle);
			...
			else
			{
				...
				bool flag3 = readOnlySpan.Equals("none", StringComparison.OrdinalIgnoreCase);
				if (flag3)
				{
					angle = Angle.None();
					flag2 = true;
				...
				else
				{
					...
					while (i < readOnlySpan.Length)
					{
						...
						bool flag4 = char.IsNumber(c) || c == '.' || c == '-';
						if (flag4)
						{
							num++;
							i++;
					...
					string text = ((num2 > 0) ? readOnlySpan.Slice(num2, readOnlySpan.Length - num2).ToString() : "deg");
					float num3;
					...
					AngleUnit angleUnit;
					bool flag7 = Angle.s_AngleUnitLookup.TryGetValue(text, out angleUnit);
					if (flag7)
					{
					...
					{
						bool flag8 = text.Equals("none", StringComparison.OrdinalIgnoreCase);
						if (!flag8)
						{
							return false;
						}
