...
namespace UnityEngine.UIElements
{
	...
	public abstract class BaseCompositeField<TValueType, TField, TFieldValue> : BaseField<TValueType>, IDelayedField where TField : TextValueField<TFieldValue>, new()
	{
		...
		public bool isDelayed
		{
			get
			{
				return this.m_IsDelayed;
			}
			set
			{
				bool flag = this.m_IsDelayed == value;
				if (!flag)
				{
					this.m_IsDelayed = value;
					foreach (TField tfield in this.fields)
					{
						tfield.isDelayed = this.m_IsDelayed;
					}
					base.NotifyPropertyChanged(in BaseCompositeField<TValueType, TField, TFieldValue>.isDelayedProperty);
				}
			}
		}
			...
			: base(label, null)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				{
					BaseCompositeField<TValueType, TField, TFieldValue>.<>c__DisplayClass23_0 CS$<>8__locals1 = new BaseCompositeField<TValueType, TField, TFieldValue>.<>c__DisplayClass23_0();
					CS$<>8__locals1.<>4__this = this;
					CS$<>8__locals1.desc = array[j];
					BaseCompositeField<TValueType, TField, TFieldValue>.<>c__DisplayClass23_0 CS$<>8__locals2 = CS$<>8__locals1;
		...
		public new abstract class UxmlSerializedData : BaseField<TValueType>.UxmlSerializedData
		{
			...
			public new static void Register()
			{
				...
				UxmlDescriptionCache.RegisterType(typeof(BaseCompositeField<TValueType, TField, TFieldValue>.UxmlSerializedData), new UxmlAttributeNames[]
				{
			...
			public override void Deserialize(object obj)
			{
				...
				bool flag = UnityEngine.UIElements.UxmlSerializedData.ShouldWriteAttributeValue(this.isDelayed_UxmlAttributeFlags);
				if (flag)
				{
					baseCompositeField.isDelayed = this.isDelayed;
				}
			}
