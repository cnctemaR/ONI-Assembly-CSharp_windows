...
namespace UnityEngine.UIElements
{
	internal class DefaultDragAndDropClient : DragAndDropData, IDragAndDrop
	{
		...
		public override IEnumerable<Object> unityObjectReferences
		{
			get
			{
				foreach (EntityId entityId in this.m_EntityIds)
				{
					yield return Object.FindObjectFromInstanceID(entityId);
					entityId = default(EntityId);
				}
				IEnumerator<EntityId> enumerator = null;
				yield break;
				yield break;
			}
		}

		public override IReadOnlyList<EntityId> entityIds
		{
			get
			{
		...
		{
			bool flag = args.entityIds != null;
			if (flag)
			{
				this.m_EntityIds = args.entityIds;
			}
