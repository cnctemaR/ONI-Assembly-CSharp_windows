...
namespace UnityEngine.UIElements
{
	internal class InlineStyleAccess : StyleValueCollection, IStyle
	{
		...
		StyleRatio IStyle.aspectRatio
		{
			get
			{
				return base.GetStyleRatio(StylePropertyId.AspectRatio);
			}
			set
			{
				bool flag = this.SetStyleValue(StylePropertyId.AspectRatio, value);
				if (flag)
				{
					this.ve.IncrementVersion(VersionChangeType.Layout | VersionChangeType.Styles);
					this.ve.layoutNode.AspectRatio = this.ve.computedStyle.aspectRatio;
				}
			}
		}
		...
		StyleMaterialDefinition IStyle.unityMaterial
		{
			get
			{
				return base.GetStyleMaterialDefinition(StylePropertyId.UnityMaterial);
			}
			set
			{
				bool flag = this.SetStyleValue(StylePropertyId.UnityMaterial, value);
				if (flag)
				{
		...
		public void ApplyInlineStyles(ref ComputedStyle computedStyle)
		{
			...
			if (flag)
			{
				InlineStyleAccess.s_StylePropertyReader.SetInlineContext(this.m_InlineRule.sheet, this.m_InlineRule.rule.properties, 1f);
			...
			bool hasInlineFilter = this.m_HasInlineFilter;
			if (hasInlineFilter)
			{
				computedStyle.ApplyStyleFilter(this.ve.style.filter.value);
			}
		}
		...
		StyleTextAutoSize IStyle.unityTextAutoSize
		{
			get
			{
				...
				StyleTextAutoSize styleTextAutoSize2;
				if (flag)
				{
					styleTextAutoSize2 = styleTextAutoSize;
				}
				else
				{
					styleTextAutoSize2 = StyleKeyword.Null;
				}
				return styleTextAutoSize2;
			}
			set
			{
		...
		StyleList<FilterFunction> IStyle.filter
		{
			get
			{
				StyleList<FilterFunction> styleList = default(StyleList<FilterFunction>);
				bool flag = this.TryGetInlineFilter(ref styleList);
				StyleList<FilterFunction> styleList2;
				if (flag)
				{
					styleList2 = styleList;
				}
				else
				{
					styleList2 = StyleKeyword.Null;
				}
				return styleList2;
			}
			set
			{
				bool flag = this.SetInlineFilter(value);
				if (flag)
				{
					this.ve.IncrementVersion(VersionChangeType.Styles | VersionChangeType.Repaint);
				}
		...
		private bool SetStyleValue(StylePropertyId id, StyleMaterialDefinition inlineValue)
		{
			...
			if (flag)
			{
				Material material = (styleValue.resource.IsAllocated ? (styleValue.resource.Target as Material) : null);
				bool flag2 = material == inlineValue.value && styleValue.keyword == inlineValue.keyword;
				if (flag2)
				{
					return false;
				}
		...
		private bool SetInlineTextAutoSize(StyleTextAutoSize inlineValue)
		{
			...
			if (flag4)
			{
				...
				flag5 = this.RemoveInlineStyle(StylePropertyId.UnityTextAutoSize);
			}
			else
			{
		...
		private bool SetInlineFilter(StyleList<FilterFunction> inlineValue)
		{
			...
			if (flag)
			{
				bool flag2 = styleList.value == inlineValue.value && styleList.keyword == inlineValue.keyword;
				if (flag2)
				{
					return false;
				}
			...
			{
				this.m_HasInlineBackgroundSize = false;
				flag5 = this.RemoveInlineStyle(StylePropertyId.Filter);
			}
			else
			{
				this.m_InlineFilter = inlineValue;
				this.m_HasInlineFilter = true;
				this.ApplyStyleFilter(inlineValue);
				flag5 = true;
			}
			return flag5;
		}
		...
		private void ApplyStyleFilter(StyleList<FilterFunction> filter)
		{
			...
			if (flag2)
			{
				flag = ComputedStyle.StartAnimationInlineFilter(this.ve, this.ve.computedStyle, filter, computedTransitionProperty.durationMs, computedTransitionProperty.delayMs, computedTransitionProperty.easingCurve);
			}
			else
			{
				this.ve.styleAnimation.CancelAnimation(StylePropertyId.TransformOrigin);
			...
			if (flag3)
			{
				this.ve.computedStyle.ApplyStyleFilter(filter.value);
			}
		}
		...
		internal struct InlineRule
		{
			public StyleProperty[] properties
			{
				get
				{
					StyleRule styleRule = this.rule;
					return (styleRule != null) ? styleRule.properties : null;
				}
			}
