	...
	internal static class ShorthandApplicator
	{
		...
		private static void CompileBackgroundPosition(StylePropertyReader reader, out BackgroundPosition backgroundPositionX, out BackgroundPosition backgroundPositionY)
		{
			...
			else
			{
				...
				if (flag7)
				{
					bool flag8 = (value.handle.valueType == StyleValueType.Dimension || value.handle.valueType == StyleValueType.Float) && (stylePropertyValue.handle.valueType == StyleValueType.Dimension || stylePropertyValue.handle.valueType == StyleValueType.Float);
					if (flag8)
					{
