...
namespace UnityEngine.UIElements.StyleSheets.Syntax
{
	...
	internal class StyleSyntaxParser
	{
		...
		private Expression ParseDataType(StyleSyntaxTokenizer tokenizer)
		{
			...
			StyleSyntaxParser.EatSpace(tokenizer);
			styleSyntaxToken = tokenizer.current;
			bool flag4 = styleSyntaxToken.type == StyleSyntaxTokenType.OpenBracket;
			if (flag4)
			{
				tokenizer.MoveNext();
				this.ParseLimits(tokenizer, out expression.min, out expression.max);
				styleSyntaxToken = tokenizer.current;
			}
			else
			{
			...
			bool flag5 = styleSyntaxToken.type != StyleSyntaxTokenType.GreaterThan;
			if (flag5)
			{
				throw new Exception(string.Format("Unexpected token '{0}' in data type expression. Expected '>' token", styleSyntaxToken.type));
			}
		...
		{
			StyleSyntaxToken styleSyntaxToken = tokenizer.current;
			bool flag = styleSyntaxToken.type != StyleSyntaxTokenType.Number;
			if (flag)
			{
				throw new Exception(string.Format("Unexpected token '{0}' in expression. Expected number token", styleSyntaxToken.type));
			}
			min = styleSyntaxToken.number;
			styleSyntaxToken = tokenizer.MoveNext();
			bool flag2 = styleSyntaxToken.type != StyleSyntaxTokenType.Comma;
			if (flag2)
			{
				throw new Exception(string.Format("Unexpected token '{0}' in expression. Expected coma", styleSyntaxToken.type));
			}
			styleSyntaxToken = tokenizer.MoveNext();
			bool flag3 = styleSyntaxToken.type != StyleSyntaxTokenType.Number;
			if (flag3)
			{
				throw new Exception(string.Format("Unexpected token '{0}' in expression. Expected number token", styleSyntaxToken.type));
			}
			...
			styleSyntaxToken = tokenizer.MoveNext();
			bool flag4 = styleSyntaxToken.type != StyleSyntaxTokenType.CloseBracket;
			if (flag4)
			{
				throw new Exception(string.Format("Unexpected token '{0}' in expression. Expected ']' ", styleSyntaxToken.type));
			}
			tokenizer.MoveNext();
		}
