	...
	internal class Page : IDisposable
	{
		...
		public class DataSet<T> : IDisposable where T : struct
		{
			public DataSet(Utility.GPUBufferType bufferType, uint totalCount, uint maxQueuedFrameCount, uint updateRangePoolSize)
			{
				...
				this.cpuData = new NativeArray<T>((int)totalCount, Page.DataSet<T>.s_CpuMemoryLabel, NativeArrayOptions.UninitializedMemory);
				...
				this.updateRanges = new NativeArray<GfxUpdateBufferRange>((int)num, Page.DataSet<T>.s_RangesMemoryLabel, NativeArrayOptions.UninitializedMemory);
			...
			private static readonly MemoryLabel s_CpuMemoryLabel = new MemoryLabel("UIElements", "Renderer.RendererCpuData", Allocator.Persistent);

			private static readonly MemoryLabel s_RangesMemoryLabel = new MemoryLabel("UIElements", "Renderer.GfxUpdateBufferRange", Allocator.Persistent);
