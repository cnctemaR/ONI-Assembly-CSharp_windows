	...
	public class VisualElementFocusRing : IFocusRing
	{
		...
		public Focusable GetNextFocusable(Focusable currentFocusable, FocusChangeDirection direction)
		{
			...
			else
			{
				...
				{
					focusable = this.GetNextFocusableInSequence(currentFocusable, direction);
				}
			}
			return focusable;
		}
		...
		internal VisualElement GetNextFocusableInTree(VisualElement currentFocusable)
		{
			...
			{
				VisualElement visualElement2 = this.GetNextElementDepthFirst(currentFocusable);
				while (!visualElement2.canGrabFocus || visualElement2.tabIndex < 0 || visualElement2.excludeFromFocusRing)
				{
					visualElement2 = this.GetNextElementDepthFirst(visualElement2);
					bool flag2 = visualElement2 == currentFocusable;
					if (flag2)
					{
		...
		internal VisualElement GetPreviousFocusableInTree(VisualElement currentFocusable)
		{
			...
			{
				VisualElement visualElement2 = this.GetPreviousElementDepthFirst(currentFocusable);
				while (!visualElement2.canGrabFocus || visualElement2.tabIndex < 0 || visualElement2.excludeFromFocusRing)
				{
					visualElement2 = this.GetPreviousElementDepthFirst(visualElement2);
					bool flag2 = visualElement2 == currentFocusable;
					if (flag2)
					{
						return currentFocusable;
