...
{
	internal class LowerCamelCaseMutator : INamingConventionMutator
	{
		public unsafe bool TryMutate(ReadOnlySpan<byte> source, Span<byte> destination, out int written)
		{
			...
			int num = 0;
			*destination[num++] = NamingConventionMutator.ToLower(*source[0]);
		...
		public unsafe bool TryMutate(ReadOnlySpan<char> source, Span<char> destination, out int written)
		{
			...
			int num = 0;
			*destination[num++] = char.ToLowerInvariant((char)(*source[0]));
			for (int i = 1; i < source.Length; i++)
			{
