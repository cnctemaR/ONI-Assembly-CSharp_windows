...
namespace LibNoiseDotNet.Graphics.Tools.Noise.Filter
{
	public class MultiFractal : FilterModule, IModule3D, IModule2D, IModule
	{
		public float GetValue(float x, float y, float z)
		{
			...
			{
				float num3 = this._offset + this._source3D.GetValue(x, y, z) * this._spectralWeights[num2];
				num *= num3;
				x *= this._lacunarity;
				y *= this._lacunarity;
				z *= this._lacunarity;
		...
		public float GetValue(float x, float y)
		{
			...
			while ((float)num2 < this._octaveCount)
			{
				float num3 = this._offset + this._source2D.GetValue(x, y) * this._spectralWeights[num2];
				num *= num3;
				x *= this._lacunarity;
				y *= this._lacunarity;
				num2++;
